...
class GRL_MLP(nn.Module):
    def __init__(self, options, inp_dim):
        super(GRL_MLP, self).__init__()
    ...
    def forward(self, x):
        # Apply Gradient Adversarial Layer
...
class temp_softmax_MLP(nn.Module):
    def __init__(self, options, inp_dim):
        super(temp_softmax_MLP, self).__init__()
        ...
        self.dnn_use_laynorm_inp = strtobool(options["dnn_use_laynorm_inp"])
        self.dnn_use_batchnorm_inp = strtobool(options["dnn_use_batchnorm_inp"])
        self.dnn_multi_constant = list(map(float, options["dnn_multi_constant"].split(",")))
        self.dnn_act = options["dnn_act"].split(",")
    ...
    def forward(self, x):
        ...
        for i in range(self.N_dnn_lay):
            ...
            if self.dnn_use_laynorm[i] and not (self.dnn_use_batchnorm[i]):
                x = self.drop[i](self.act[i](self.ln[i](self.wx[i](x)) * self.dnn_multi_constant[i]))

            if self.dnn_use_batchnorm[i] and not (self.dnn_use_laynorm[i]):
                x = self.drop[i](self.act[i](self.bn[i](self.wx[i](x)) * self.dnn_multi_constant[i]))

            if self.dnn_use_batchnorm[i] == True and self.dnn_use_laynorm[i] == True:
                x = self.drop[i](self.act[i](self.bn[i](self.ln[i](self.wx[i](x))) * self.dnn_multi_constant[i]))

            if self.dnn_use_batchnorm[i] == False and self.dnn_use_laynorm[i] == False:
                x = self.drop[i](self.act[i](self.wx[i](x) * self.dnn_multi_constant[i]))
...
class GRU_drop_forward(nn.Module):
    def __init__(self, options, inp_dim):
        super(GRU_drop_forward, self).__init__()
