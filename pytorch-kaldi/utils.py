...
def cfg_item2sec_no_error(config, field, value):
...
def write_cfg_chunk(
    ...
    # use one gpu during forward if gpu is used (because muti gpu won't improve the forward speed)
    if config_chunk["exp"]["to_do"] == 'forward':
        config_chunk["exp"]["multi_gpu"] = 'False'
...
def dict_fea_lab_arch(config, fea_only):
    ...
    fea_lst = list(re.findall("fea_name=(.*)\n", config["data_chunk"]["fea"].replace(" ", "")))
    lab_lst = list(re.findall("lab_name=(.*)\n", config["data_chunk"]["lab"].replace(" ", "")))
    # fea_lst_used = [[fea_name, fea_lst, fea_opts, cw_left, cw_right]]
    # fea_lst_used = [['mfcc', '/the/path/*.lst', 'apply-cmvn --utt2spk=ark:/the/path/utt2spk  scp:/the/path/cmvn.scp ark:- ark:- | add-deltas ark:- ark:- |', '0', '0']]
    fea_lst_used = []
    # lab_lst_used = [[lab_name, lab_folder, lab_opts]]
    # lab_lst_used = [['lab_cd', '/the/path/tri1_ali_train_si284/', 'ali-to-pdf']]
    lab_lst_used = []
    # arch_lst_used = [arch_id, arch_name, arch_seq_model]
    # arch_lst_used = ['architecture1', 'LSTM_layers', 'True']
    arch_lst_used = []

    # fea_dict_used = {fea_name: [fea_name, fea_lst, fea_opts, cw_left, cw_right]}
    # fea_dict_used = {'mfcc': ['mfcc', '/the/path/*.lst', 'apply-cmvn --utt2spk=ark:/the/path/utt2spk  scp:/the/path/cmvn.scp ark:- ark:- | add-deltas ark:- ark:- |', '0', '0']}
    fea_dict_used = {}
    # lab_dict_used = {lab_name: [lab_name, lab_folder, lab_opts]}
    # lab_dict_used = {'lab_cd': ['lab_cd', '/the/path/tri1_ali_train_si284/', 'ali-to-pdf']}
    lab_dict_used = {}
    # arch_dict_used = {arch_name: [arch_id, arch_name, arch_seq_model]}
    # arch_dict_used = {'LSTM_layers': ['architecture1', 'LSTM_layers', 'True']}
    arch_dict_used = {}

    # fea_lst_used_name = [fea_name]
    # fea_lst_used_name = ['mfcc']
    fea_lst_used_name = []
    # lab_lst_used_name = [lab_name]
    # lab_lst_used_name = ['lab_cd']
    lab_lst_used_name = []
    # arch_lst_used_name = [arch_name]
    # arch_lst_used_name = ['LSTM_layers']
    arch_lst_used_name = []
...
def model_init(inp_out_dict, model, config, arch_dict, use_cuda, multi_gpu, to_do):
    def _soft_nll(inputs, target, reduction='mean'):
    ...
    for line in model:
        ...
        if operation == "condition_soft_label":

            inp_dim1 = inp_out_dict[inp1][-1]

            inp_out_dict[out_name] = [inp_dim1]

        if operation == "cost_nll" or operation == "cost_nll_semi" or operation == "cost_nll_2class":
            costs[out_name] = nn.NLLLoss()
            inp_out_dict[out_name] = [1]

        if operation == "cost_soft_nll":
            costs[out_name] = _soft_nll
            inp_out_dict[out_name] = [1]
        if operation == "cost_soft_vector_nll":
            costs[out_name] = _soft_nll
            inp_out_dict[out_name] = [1]
        if operation == "cost_soft_ts_nll":
            costs[out_name] = _soft_nll
            inp_out_dict[out_name] = [1]

        if operation == "cost_err":
            inp_out_dict[out_name] = [1]
        if operation == "cost_zero":
            inp_out_dict[out_name] = [1]
        ...
        if (
            ...
            or operation == "avg"
            or operation == "mse"
            or operation == "softmax"
            or operation == "real_softmax"
        ):
...
def forward_model_refac01(
    ...
    for line in model:
        ...
        if operation == "compute":
        ...
        elif operation == "condition_soft_label":
            # it won't be used, just for the completeness
            lab_dnn = _get_labels_from_input(ref, inp2, lab_dict)
            out = _get_network_output(outs_dict, inp1, max_len_lab, batch_size)

            if to_do != "forward":
                outs_dict[out_name] = out

        elif operation == "cost_nll" or operation == "cost_nll_semi":
            lab_dnn = _get_labels_from_input(ref, inp2, lab_dict)
            out = _get_network_output(outs_dict, inp1, max_len_lab, batch_size)
            if to_do != "forward":
                outs_dict[out_name] = costs[out_name](out, lab_dnn)

        elif operation == "cost_nll_2class":
            lab_dnn = _get_labels_from_input(ref, inp2, lab_dict)
            ones = torch.ones_like(lab_dnn)
            lab_dnn = torch.where(lab_dnn > 1, ones, lab_dnn)
            out = _get_network_output(outs_dict, inp1, max_len_lab, batch_size)
            if to_do != "forward":
                outs_dict[out_name] = costs[out_name](out, lab_dnn)

        elif operation == "cost_soft_nll":
            # same with cost_nll because that the soft label won't be used during forward, which is forward_model_refac01 function used currently.
            lab_dnn = _get_labels_from_input(ref, inp2, lab_dict)
            out = _get_network_output(outs_dict, inp1, max_len_lab, batch_size)
            if to_do != "forward":
                outs_dict[out_name] = costs[out_name](out, lab_dnn)
        elif operation == "cost_soft_vector_nll":
            # same with cost_nll because that the soft label won't be used during forward, which is forward_model_refac01 function used currently.
            lab_dnn = _get_labels_from_input(ref, inp2, lab_dict)
            out = _get_network_output(outs_dict, inp1, max_len_lab, batch_size)
            if to_do != "forward":
                outs_dict[out_name] = costs[out_name](out, lab_dnn)
        elif operation == "cost_soft_ts_nll":
            # same with cost_nll because that the soft label won't be used during forward, which is forward_model_refac01 function used currently.
            out2 = _get_network_output(outs_dict, inp2, max_len_lab, batch_size)
            out1 = _get_network_output(outs_dict, inp1, max_len_lab, batch_size)
            if to_do != "forward":
                outs_dict[out_name] = costs[out_name](out1, out2)
            ...
            outs_dict[out_name] = act( outs_dict[inp1] * float(inp2) )
            if to_do == "forward" and out_name == forward_outs[-1]:
                break
        elif operation == "real_softmax":
            act = nn.Softmax(dim=1)
            outs_dict[out_name] = act( outs_dict[inp1] * float(inp2) )
            if to_do == "forward" and out_name == forward_outs[-1]:
                break
        elif operation == "avg":
            outs_dict[out_name] = 0
            if to_do == "forward" and out_name == forward_outs[-1]:
                break
    return outs_dict
...
def forward_model(
    config, fea_dict, lab_dict, arch_dict, model, nns, costs, inp, inp_out_dict, max_len, batch_size, to_do, forward_outs
):
    ...
    for line in model:
        ...
        if operation == "condition_soft_label":
            ...
            lab_soft_teacher = outs_dict[inp1]

            if len(lab_soft_teacher.shape) == 3:
                lab_soft_teacher = lab_soft_teacher.view(max_len * batch_size, -1)

            if to_do != "forward":
                if cfg_item2sec_no_error(config, "arch_name", "temp_softmax_MLP_layers") != -1:
                    temp = int(1 / float(config[cfg_item2sec(config, "arch_name", "temp_softmax_MLP_layers")]["dnn_multi_constant"]))
                else:
                    temp = 1
                    pattern_softmax = "(.*)=softmax\((.*),(.*)\)"
                    for line_softmax in config["model"]["model"].split("\n"):
                        if len(re.findall(pattern_softmax, line_softmax)) != 0:
                            [_, _, inv_temp] = list(re.findall(pattern_softmax, line_softmax)[0])
                            if inv_temp != '1':
                                temp = int(1 / float(inv_temp))
                                break
                ...
                embeds.cuda()
                lab_soft_sum = embeds(lab_dnn)

                outs_dict[out_name] = torch.where(torch.argmax(lab_soft_teacher,dim=1)==lab_dnn,lab_soft_teacher.t(),lab_soft_sum.t()).t()
        ...
        if operation == "cost_nll_semi":
            # in this operation, only orig clean and real noisy are involved for loss computation, the simu noisy part are removed
            ...
            # find out whether two domain (target and source) are involved in this cost computation
            if "lab_domain_classify" in lab_dict:
                if len(inp.shape) == 3:
                    lab_domain_classify = inp[:, :, lab_dict["lab_domain_classify"][3]]
                if len(inp.shape) == 2:
                    lab_domain_classify = inp[:, lab_dict["lab_domain_classify"][3]]

                # create indices for lab_domain_classify label 0 and 1, which means orig clean and real noisy
                lab_domain_classify = lab_domain_classify.view(-1).long()
                zeros = torch.zeros_like(lab_domain_classify)
                ones = torch.ones_like(lab_domain_classify)
                lab_domain_classify = torch.where(lab_domain_classify > 1, zeros, ones)
                
                indices = torch.nonzero(lab_domain_classify).view(-1)
                # select the orig clean and real noisy part for ce loss computation
                lab_dnn = torch.index_select(lab_dnn, 0, indices)
                out = torch.index_select(out, 0, indices)

            if to_do != "forward":
                outs_dict[out_name] = costs[out_name](out, lab_dnn) 

        if operation == "cost_nll_2class":
            ...
            # convert lab_domain_classify label 2 to 1, which means make real noisy and simu noisy the same class
            ones = torch.ones_like(lab_dnn)
            lab_dnn = torch.where(lab_dnn > 1, ones, lab_dnn)
        ...
        if operation == "cost_soft_nll":
            ...
            if "lab_domain_classify" in lab_dict:
                ...
                # create indices for lab_domain_classify label 1, which means real noisy
                lab_domain_classify = lab_domain_classify.view(-1).long()
                zeros = torch.zeros_like(lab_domain_classify)
                lab_domain_classify = torch.where(lab_domain_classify > 1, zeros, lab_domain_classify)
                
                indices = torch.nonzero(lab_domain_classify).view(-1)
                
                # select the real noisy part for soft label loss computation
                lab_dnn = torch.index_select(lab_dnn, 0, indices)
                out = torch.index_select(out, 0, indices)
        ...
        if operation == "cost_soft_vector_nll":
            ...
            if to_do != "forward":
                ...
                with open("/home/zhuhan/NasStore/pytorch-kaldi/exp/chime3/forward/chime3_GRU_4layer_mfcc_train_tr05_orig_clean_forward_tr05_orig_clean/temp_" + str(temp) + "_soft_label_vector.pkl", 'rb') as f_in:
               # with open("/home/zhuhan/NasStore/pytorch-kaldi/exp/commonvoice/forward/commonvoice_GRU_4layer_mfcc_train_us_30000_forward_us_30000/temp_" + str(temp) + "_soft_label_vector.pkl", 'rb') as f_in:
                    numpy_embedding = pickle.load(f_in)
                lab_dnn = torch.from_numpy(numpy_embedding.astype(np.float32)).cuda()
                outs_dict[out_name] = costs[out_name](out, lab_dnn)
        ...
        if operation == "cost_soft_ts_nll":
            ...
            if len(out1.shape) == 3:
                out1 = out1.view(max_len * batch_size, -1)

            out2 = outs_dict[inp2]

            if len(out2.shape) == 3:
                out2 = out2.view(max_len * batch_size, -1)
