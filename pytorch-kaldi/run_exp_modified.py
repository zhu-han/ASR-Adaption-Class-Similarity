...
def _max_nr_of_parallel_forwarding_processes(config):
    if "max_nr_of_parallel_forwarding_processes" in config["forward"]:
        return np.minimum(int(config["forward"]["max_nr_of_parallel_forwarding_processes"]), len(CUDA_VISIBLE_DEVICES_LIST))
    # return -1
    return len(CUDA_VISIBLE_DEVICES_LIST)
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "--train":
        decode = False
        decode_in_new_dir = False
    elif sys.argv[1] == "--decode":
        decode = True
        decode_in_new_dir = False
    elif sys.argv[1] == "--decode-in-new-dir":
        decode = False
        decode_in_new_dir = True  
    ...
    if decode_in_new_dir:
        # create chunk config as it only has 1 epoch
        N_ep = 1 
        config["exp"]["N_epochs_tr"] = '1'
    ...
    run_nn_refac01 = getattr(module, 'run_nn_refac01')


    # Splitting data into chunks (see out_folder/exp_files)
    create_lists(config)
    ...
    if decode_in_new_dir:
        ...
        for arch in pt_files.keys():
            model_files[arch] = pt_files[arch]
        for pt_arch in pt_files.keys():
            forward_model = out_folder + "/exp_files/train_" + config["data_use"]["train_with"] + "_ep" + format(0, N_ep_str_format) + "_ck" + format(int(config["dataset1"]["N_chunks"]) - 1, "0" + str(max(math.ceil(np.log10(int(config["dataset1"]["N_chunks"]))), 1)) + "d") + "_" + pt_arch + ".pkl"
            if os.path.exists(model_files[pt_arch]) and not os.path.exists(forward_model):
                copyfile(model_files[pt_arch], forward_model)
            ...
            for ck in range(N_ck_tr):
                ...
                if do_validation_after_chunk(ck, N_ck_tr, config):
                    ...
                    if not _is_first_validation(ep,ck, N_ck_tr, config):
                        ...
                        err_valid_mean_list.append(err_valid_mean)
                        ...
                        for lr_arch in lr.keys():
                            if ep < N_ep - 1 and auto_lr_annealing[lr_arch]:
                                if ((err_valid_mean_prev - err_valid_mean) / err_valid_mean) < improvement_threshold[
                                    ...
                                    if new_lr_value < float(lr[lr_arch][0]) / 10:
    ...
    if not (decode_in_new_dir or decode or is_production):
        best_epoch = len(err_valid_mean_list) - 1 - np.argmin(err_valid_mean_list[-1:len(err_valid_mean_list)//2:-1])

        print("-----")
        print("best performance epoch is ep" + format(best_epoch, N_ep_str_format))
        print("-----")
        if finish_training:
            ep -= 1
        for pt_arch in pt_files.keys():
            model_files[pt_arch] = model_files[pt_arch].replace("ep" + format(ep, N_ep_str_format), "ep" + format(N_ep - 1, N_ep_str_format))

        for pt_arch in pt_files.keys():
            best_model = out_folder + "/exp_files/train_" + config["data_use"]["train_with"] + "_ep" + format(best_epoch, N_ep_str_format) + "_ck" + format(int(config["dataset1"]["N_chunks"]) - 1, "0" + str(max(math.ceil(np.log10(int(config["dataset1"]["N_chunks"]))), 1)) + "d") + "_" + pt_arch + ".pkl"
            if os.path.exists(best_model):
                copyfile(best_model, model_files[pt_arch])
            else:
                sys.stderr.write(
                    "ERROR: best model {} doesn't exist \n".format(best_model)
                )
                sys.exit(0)
        ...
        model_files_list = glob.glob(out_folder + "/exp_files/train*pkl")
        for pt_arch in pt_files.keys():
            model_files_list.remove(model_files[pt_arch])
        for model_path in model_files_list:
            if os.path.exists(model_path):
                os.remove(model_path)


    if decode or decode_in_new_dir or is_production:
        # --------FORWARD--------#
        for forward_data in forward_data_lst:
            ...
            for ck in range(N_ck_forward):
                os.environ['CUDA_VISIBLE_DEVICES'] = CUDA_VISIBLE_DEVICES_LIST[ck % len(CUDA_VISIBLE_DEVICES_LIST)]
                ...
                if not (os.path.exists(info_file)):
                    ...
                    if _run_forwarding_in_subprocesses(config):
                        ...
                        p.start()
                        '''
                        # This 'run_nn' script should be 'run_nn_refac01' because the previous 'read_lab_fea' script is 'read_lab_fea_refac01'
                        [data_name, data_set, data_end_index, fea_dict, lab_dict, arch_dict] = run_nn_refac01(
                            data_name,
                            data_set,
                            data_end_index,
                            fea_dict,
    ...
    if not is_production and not decode and not decode_in_new_dir:
        create_curves(out_folder, real_N_ep, valid_data_lst)
