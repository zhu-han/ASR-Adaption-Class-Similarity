...
def load_dataset(
    ...
    def _read_features_and_labels_with_kaldi(fea_scp, fea_opts, fea_only, lab_folder, lab_opts, output_folder):
        ...
        '''
        lab = {'4apc020g': array([ 0, 43, 43, ..., 44, 44, 44], dtype=int32)}
        fea = {'49zc031b': array([[-14.17585   , -10.177874  ,  -0.06176376, ...,   0.5037758 ,
                                            -0.30883318,   0.9773874 ],
                                  [-14.051983  , -12.39105   ,  -4.4194264 , ...,   0.3056342 ,
                                            0.17366385,   0.27996516],
                                  [-14.206814  , -12.39105   ,  -4.7751536 , ...,  -0.732815  ,
                                            0.891047  ,  -0.4715044 ],
                                   ...,
                                  [-15.135807  , -11.786585  ,  -8.517557  , ...,   0.461551  ,
                                           0.35068333,   0.34044522]], dtype=float32)}
        '''
        return fea, lab
...
def load_chunk(
    ...
    '''
    data_name = ['20pc0101', '48ic040u', ..., '48yc030n_split0', '48yc030n_split1', '20qc0105_split0', 
                    '20qc0105_split1', '20qc0105_split2', '20qc0105_split3']

    data_set = [[-0.76641244 -0.26279047  0.15475966 ...  1.59493554  0.73728055
                    0.        ]
                [-0.77592379 -0.27777958 -0.14182721 ... -0.31180897 -0.25141883
                    0.        ]
                [-0.85131162 -0.37520915  0.0617128  ... -1.09441864 -0.89956057
                    0.        ]
                    ...
                [-1.54829752 -0.67793095 -0.45970291 ... -0.56312203 -1.26422369
                    45.        ]
                [-1.60972905 -0.72869635 -0.65341151 ... -1.11803436 -1.57870722
                    45.        ]
                [-1.56877458 -0.84611845 -0.71080202 ... -0.47883222 -0.17606625
                    44.        ]]
    # This data_set concatenate its context feature and lable.
    # Thus the dimension of data_set.shape[1] is (context_window_size * feature_dimension + 1) 



    end_index_fea = [   126    254    383 ... 454424 455047 455672]
    '''
...
def read_lab_fea(cfg_file, fea_only, shared_list, output_folder):
    ...
    # fea_dict = {fea_name: [fea_name, fea_lst, fea_opts, cw_left, cw_right]}
    # fea_dict = {'mfcc': ['mfcc', '/the/path/*.lst', 'apply-cmvn --utt2spk=ark:/the/path/utt2spk  scp:/the/path/cmvn.scp ark:- ark:- | add-deltas ark:- ark:- |', '0', '0']}

    # lab_dict = {lab_name: [lab_name, lab_folder, lab_opts]}
    # lab_dict = {'lab_cd': ['lab_cd', '/the/path/tri1_ali_train_si284/', 'ali-to-pdf']}

    # arch_dict = {arch_name: [arch_id, arch_name, arch_seq_model]}
    # arch_dict = {'LSTM_layers': ['architecture1', 'LSTM_layers', 'True']}
    [fea_dict, lab_dict, arch_dict] = dict_fea_lab_arch(config, fea_only)
    [cw_left_max, cw_right_max] = compute_cw_max(fea_dict)

    fea_index = 0
    # denote different fearutes to be concatenated (eg: mfcc + fbank)
    cnt_fea = 0
    for fea in fea_dict.keys():
        ...
        # denote different labels to be concatenated (eg: context-dependent states + monophone)
        cnt_lab = 0
    ...
    '''
    # fea_dict = {fea_name: [fea_name, fea_lst, fea_opts, cw_left, cw_right, start_index_in_data_set_1_axis(included), end_index_in_data_set_1_axis(not included), feature_dimension_after_concatenating_contex]}
    # fea_dict = {'mfcc': ['mfcc', '/the/path/*.lst', 'apply-cmvn --utt2spk=ark:/the/path/utt2spk  scp:/the/path/cmvn.scp ark:- ark:- | add-deltas ark:- ark:- |', '0', '0', 0, 39, 39]}

    # lab_dict = {lab_name: [lab_name, lab_folder, lab_opts, label_index_in_data_set_1_axis]}
    # lab_dict = {'lab_cd': ['lab_cd', '/the/path/tri1_ali_train_si284/', 'ali-to-pdf', 39]}
    ...
    data_end_index  = [   126    255    385 ... 460225 460847 461471]
    len(data_end_index) = 1062

     data_set =
        [[-7.63069605e-03  7.24672139e-01  1.41906333e+00 ... -5.27951121e-01
        -1.59795177e+00  1.55200000e+03]
        [-2.59098917e-01  8.91556501e-01  1.28337336e+00 ... -3.31654489e-01
        -1.34884667e+00  3.31000000e+02]
        [-4.80982363e-01  6.79158211e-01  1.79899490e+00 ...  2.66366005e-01
        -1.29523262e-01  3.31000000e+02]
        ...
        [-1.32163119e+00 -6.30257666e-01  8.68045166e-02 ... -1.14530540e+00
        1.76651156e+00  4.50000000e+01]
        [-1.25338244e+00 -5.23136139e-01  1.58822864e-01 ... -3.03078711e-01
        7.71124363e-01  4.50000000e+01]
        [-1.31783962e+00 -6.19545460e-01 -2.57283211e-01 ...  3.59421313e-01
        -3.51430923e-01  4.40000000e+01]]
    # This data_set concatenate the context feature and lable of different features and differen lables.
    # Thus the dimension of data_set.shape[1] is (num_different_feature * context_window_size * feature_dimension + num_different_label * 1) 
    len(data_set) = 461471
    len(data_set[0]) = 40
    '''
